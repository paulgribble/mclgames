...
i1 = 0.025
i2 = 0.075
slim = [-45*math.pi/180.0, 160.0*math.pi/180.0]
elim = [5*math.pi/180.0, 160.0*math.pi/180.0]
limzone = 1*math.pi/180.0
...
while done==False:
    ...
    limtorel = 0.0
    if (s-slim[0] < limzone):
        limtorsh = abs(s-slim[0])*limk*2 - abs(sd)*limd
    if (slim[1]-s < limzone):
        limtorsh = -abs(slim[1]-s)*limk*2 + abs(sd)*limd
    if (e-elim[0] < limzone):
        limtorel = abs(e-elim[0])*limk - abs(ed)*limd
    if (elim[1]-e < limzone):
        limtorel = -abs(elim[1]-e)*limk + abs(ed)*limd

#    print '-------------------'
#    print sf,se,ef,ee
#    print torque[0,0],torque[0,1]
#    print limtorsh,limtorel
#    print round(s,2),round(e,2),round(sd,2),round(ed,2),round(sdd,2),round(edd,2)
#    print '-------------------'

    torque[0,0] = torque[0,0] + limtorsh
    torque[0,1] = torque[0,1] + limtorel
    ...
    sdd = acc[0,0]
    if abs(sdd) > maxacc:
        sdd = math.copysign(maxacc,sdd)
    edd = acc[1,0]
    if abs(edd) > maxacc:
        edd = math.copysign(maxacc,edd)
